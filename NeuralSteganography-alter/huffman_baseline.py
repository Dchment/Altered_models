...
def encode_huffman(model, enc, message, context, bits_per_word, finish_sent=False, device=device):
    length = len(message)
...
def decode_huffman(model, enc, text, context, bits_per_word, device=device ):
