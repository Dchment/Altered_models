...
class Dictionary(object):#词典类
    ...
    def build_vocabulary(self):#构建词典
        counter = collections.Counter(self._words)#构建词与其词频的dict
...
def prepare_data(data_raw, params):#用语料库构建样本、标签、词向量词典、词典
    ...
    embed_arr=torch.tensor(embed_arr)
    data=[]
    labels=[]
    for sent in data_dict.sentences:
        if (len(sent) < params.line_max_size - 2):
            data.append([data_dict.word2idx[word] for word in sent[:-1]])#样本为语句第一个词到倒数第二个词
            labels.append([data_dict.word2idx[word] for word in sent[1:]])#标签为语句第二个词到倒数第一个词（与样本一一对应）
