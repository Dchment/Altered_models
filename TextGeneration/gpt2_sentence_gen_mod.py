...
class Steganography():
    ...
    def encode(self,tokenizer=tokenizer,model=model,context=context,start_point=0):
        start=start_point
        exceeded_words=0#为了隐写文本完整性而多生成的符号数
        predicted_index=0
        ...
        self.exceeded=exceeded_words
        '''
        exceeded_words=bin(exceeded_words).replace('0b','')+'b'
        bit=exceeded_words+bit
        '''
        self.text_bit=bit
    ...
    def decode(self,text,tokenizer=tokenizer,model=model,context=context):
        ...
        start_point=len(tokenizer.encode(context))#跳过语境文本开始解码
        targettext = ''
        with torch.no_grad():
